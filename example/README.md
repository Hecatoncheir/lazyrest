...
recursive variables, and a request that uses what an earlier one answered. `streams.http` demonstrates a WebSocket request, and `cache.socket` a raw
TCP request against a local Redis.
