...
func FromConfig(config Config) (Theme, error) {
	...
	settings := []struct {
		...
		{config.Muted, []*tcell.Color{&result.Tree.Title, &result.Suites.Title, &result.Suites.SuiteForeground, &result.Suite.Title, &result.Producer.Title, &result.Footer.Foreground, &result.Syntax.Punctuation, &result.Syntax.Comment}},
		{config.Accent, []*tcell.Color{&result.Tree.TitleFocus, &result.Tree.NodeDirectory.Foreground, &result.Suites.TitleFocus, &result.Suite.TitleFocus, &result.Producer.TitleFocus, &result.Syntax.Key}},
		...
		{config.Progress, []*tcell.Color{&result.Footer.SuiteBackground, &result.Syntax.Number, &result.Syntax.Variable}},
		{config.ProgressForeground, []*tcell.Color{&result.Footer.SuiteForeground}},
		{config.Success, []*tcell.Color{&result.Footer.SuiteSuccess.Background, &result.Syntax.String}},
		{config.SuccessForeground, []*tcell.Color{&result.Footer.SuiteSuccess.Foreground}},
		{config.Failure, []*tcell.Color{&result.Footer.SuiteFailure.Background, &result.Syntax.Literal, &result.Syntax.Keyword}},
