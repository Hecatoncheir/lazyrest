...
func TestApplySuiteThemeUpdatesFilledTextBackground(t *testing.T) {
	uiTheme := theme.SuiteTheme{
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			view := tview.NewTextView().SetText("request")
			view.SetBackgroundColor(tcell.NewHexColor(0x504945))
			view.SetRect(0, 0, 12, 3)
			applySuiteTheme(view, uiTheme, test.focused)
